...
export const HistoryPage: React.FC = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || 'Failed to fetch invoices history');
    } finally {
      setIsLoading(false);
    }
  }, [activeShop]);
  ...
  const filteredInvoices = invoices.filter((inv) => {
    ...
    const matchesStatus = paymentStatus ? inv.paymentStatus === paymentStatus : true;
    const matchesStartDate = startDate ? inv.invoiceDate >= startDate : true;
    const matchesEndDate = endDate ? inv.invoiceDate <= endDate : true;

    return matchesSearch && matchesCustomer && matchesStatus && matchesStartDate && matchesEndDate;
  });
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 border border-slate-250 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-market-700 bg-slate-50 dark:bg-slate-880"
            />
          </div>
          ...
          <Select
            ...
            options={[
              { value: '', label: 'All Payments Statuses' },
      ...
      {isLoading ? (
        <div className="p-12 text-center text-sm font-bold text-slate-400 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100">
          Loading invoice logs...
        </div>
      ) : filteredInvoices.length === 0 ? (
        <Card>
          <EmptyState
            title="No Invoices Found"
            description="No historical invoice records match your criteria."
          />
        </Card>
      ) : (
