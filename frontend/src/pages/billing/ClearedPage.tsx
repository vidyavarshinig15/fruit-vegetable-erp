...
export const ClearedPage: React.FC = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        // Only get PAID bills that are not cancelled
        const paidList = (res.data.data || []).filter(
          (inv: any) => inv.paymentStatus === 'PAID' && inv.billStatus !== 'CANCELLED'
        );
        setInvoices(paidList);
      }
    } catch (err: any) {
      setErrorMsg(err.response?.data?.message || 'Failed to fetch cleared invoices');
    } finally {
      setIsLoading(false);
    }
  }, [activeShop]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <h1 className="text-3xl font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-tight flex items-center gap-2">
            <FileText className="w-8 h-8 text-emerald-750" />
            Fully Cleared Bills
          </h1>
      ...
      {isLoading ? (
        <div className="p-12 text-center text-sm font-bold text-slate-400 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100">
          Loading fully cleared bills...
        </div>
      ) : filteredInvoices.length === 0 ? (
        <Card>
          <EmptyState
            title="No Cleared Bills Yet"
            description="Fully settled customer bills will be archived here."
          />
        </Card>
      ) : (
