...
export const PartialPage: React.FC = () => {
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        // Only get PARTIALLY_PAID bills that are not cancelled
        const partialList = (res.data.data || []).filter(
          (inv: any) => inv.paymentStatus === 'PARTIALLY_PAID' && inv.billStatus !== 'CANCELLED'
        );
        setInvoices(partialList);
      }
    } catch (err: any) {
      setErrorMsg(err.response?.data?.message || 'Failed to fetch partially paid invoices');
    } finally {
      setIsLoading(false);
    }
  }, [activeShop]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <h1 className="text-3xl font-black text-sky-700 dark:text-sky-400 uppercase tracking-tight flex items-center gap-2">
      ...
      {isLoading ? (
        <div className="p-12 text-center text-sm font-bold text-slate-400 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100">
          Loading partially paid bills...
        </div>
      ) : filteredInvoices.length === 0 ? (
        <Card>
          <EmptyState
            title="No Partially Paid Bills"
            description="No partially paid bills found."
          />
        </Card>
      ) : (
